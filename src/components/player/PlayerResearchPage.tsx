...
import {
  ...
  getDvpLabel,
  getOpponentOptions,
  getPlayerResearchEndpoint,
  getPlayerResearchHitRate,
  ordinal,
  rankToEdgeScore,
  resolvePlayerResearchRequest,
...
export default function PlayerResearchPage() {
  ...
  const id = params.id;
  const request = resolvePlayerResearchRequest(id, searchParams);
  const { league, requestedDirection, opponent, playerName, queryTeam, oddsEventId, requestedPlayerId, requestedPropType, requestedLine } = request;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const response = await fetch(getPlayerResearchEndpoint(id, request));
      ...
      <header className="sticky top-0 z-40 border-b border-dark-border bg-dark-bg/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="text-right">
            <p className="text-[11px] uppercase tracking-[0.24em] text-gray-500">Player Research</p>
