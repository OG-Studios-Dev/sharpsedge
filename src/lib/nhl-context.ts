    ...
    meta: {
      sources: {
        ...
        specialTeams: {
          ...
          degraded: ppMap.size < 30 || pkMap.size < 30,
          note: "PP/PK sourced from NHL stats REST API (season aggregates). Goalie EV/PP/SH save breakdown from savesByStrength. HIGH-DANGER zone SV% now available via nhl-pbp-aggregate (PBP x/y coordinates, last 10 games). EV/PP/SH strength splits from savesByStrength.",
        },
      },
      notes: [
        ...
        "Goalie EV/PP/SH strength splits sourced from NHL stats REST API (savesByStrength). HIGH-DANGER zone SV% (HDSV%) now derived from nhl-pbp-aggregate (play-by-play x/y coordinates, last 10 games).",
