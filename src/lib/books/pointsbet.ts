...
import { extractPointsBetMLBF5Odds } from "@/lib/books/mlb-f5-augment";
...
const PATHS: Partial<Record<AggregatedSport, string>> = {
  ...
  MLB: "/7158/events/featured?includeLive=false",
  NFL: "/7157/events/featured?includeLive=false",
  EPL: "/7228/events/featured?includeLive=false",
  SERIE_A: "/7229/events/featured?includeLive=false",
  PGA: "/7183/events/featured?includeLive=false",
};
...
export async function fetchOdds(sport: AggregatedSport): Promise<BookEventOdds[]> {
  ...
  try {
    ...
    return events.flatMap((event: any) => {
      ...
      const extras = sport === "MLB" ? extractPointsBetMLBF5Odds(event) : [];
