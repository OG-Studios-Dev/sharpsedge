...
const SIGNAL_PATTERNS: Record<GooseSignal, RegExp[]> = {
  ...
  travel_fatigue: [
    /travel\s+(?:fatigue|wear|toll|disadvantage)/i,
    /\bfatigue[d]?\b.*(?:travel|road|trip)/i,
    /back-to-back.*road/i,
    /long.*road\s+trip/i,
    /cross[\s-]country/i,
    /\btraveled\b.*(?:miles|coast|flight)/i,
  ],
  back_to_back: [
    /back[\s-]to[\s-]back/i,
    /\bB2B\b/i,
  ...
  streak_form: [
    /win(?:ning)?\s+streak/i,
    /losing\s+streak/i,
    /\bhot\s+(?:streak|run|stretch)\b/i,
    /\bcold\s+(?:streak|stretch|spell)\b/i,
    /recent\s+(?:form|winning|run)/i,
    /\b(?:won|lost)\s+\d+\s+(?:straight|in\s+a\s+row)/i,
  ],
  goalie_news: [
    /goalie/i,
    /\bnetminder\b/i,
  ...
  lineup_change: [
    /lineup\s+(?:change|shakeup|shuffle|update|adjustment)/i,
    /\bscratch(?:ed)?\b/i,
    /\binserted\b.*(?:lineup|starting)/i,
  ...
  matchup_edge: [
    /favorable.*matchup/i,
    /matchup.*(?:edge|advantage)/i,
    /head[\s-]to[\s-]head/i,
    /\bH2H\b/i,
    /\bmatchup\s+(?:history|data|splits)/i,
  ],
  weather: [
    /\bweather\s+(?:impact|factor|concern|advantage|conditions)\b/i,
    /\bprecipitation\b/i,
  ],
  park_factor: [
    /park factor/i,
  ...
  bullpen_strength: [
    /bullpen\s+(?:strength|advantage|edge|depth)/i,
    /\bstrong\s+(?:bullpen|pen|relievers?)\b/i,
    /\bshutdown\s+(?:bullpen|closer|relievers?)\b/i,
    /relief.*pitching\s+(?:strength|advantage)/i,
  ],
  injury_news: [
    /injur/i,
    /\bIR\b/i,
