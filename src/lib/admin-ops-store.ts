...
const DEFAULT_DATA: AdminOpsData = {
  ...
  cronSchedules: [
    ...
    {
      id: "cron_goose2_daily_verify",
      name: "Goose2 daily verification",
      schedule: "25 17 * * *",
      purpose: "Verify that the daily learning-model ingestion wrote fresh snapshot and candidate rows and did not leave stale pending qualifiers behind.",
      owner: "Goose2 pipeline",
      target: "goose_market_candidates / goose_market_results / system_qualifiers",
      path: "/api/admin/goose2/verify-daily?cron=true",
      enabled: true,
      lastRunAt: null,
      lastSuccessAt: null,
      lastFailureAt: null,
      consecutiveFailures: 0,
      notes: "Runs shortly after the hourly multi-sport snapshot capture. Opens an admin incident if stored data freshness falls behind or stale pending qualifiers accumulate.",
      updatedAt: nowIso(),
    },
  ],
  incidents: [],
