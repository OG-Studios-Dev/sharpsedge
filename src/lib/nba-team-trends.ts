...
export async function buildNBATeamTrends(games: NBAGame[]): Promise<TeamTrend[]> {
  ...
  for (const {
    ...
    // NOTE: Do NOT generate synthetic 1Q team totals from overall team win rate.
    // That signal produced fake "L10 / 68%" style picks with no real quarter-data backing.
    // If we want 1Q markets here again, they must be sourced from verified quarter-level stats.
  }
