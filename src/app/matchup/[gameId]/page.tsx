...
export default async function MatchupPage({ params }: { params: { gameId: string } }) {
  const data = await getNHLMatchupData(params.gameId);
    ...
    return (
      <main className="min-h-screen bg-dark-bg px-4 py-10 text-white md:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl rounded-[32px] border border-white/10 bg-white/[0.04] p-6 text-center shadow-[0_24px_80px_rgba(0,0,0,0.28)]">
          <p className="text-[11px] uppercase tracking-[0.24em] text-gray-500">NHL</p>
  ...
  return <CompactMatchupPageClient data={data} title="Matchup" backHref="/schedule" />;
}
