-- Season/year-partitioned context builder for full historical Ask Goose coverage.
-- The first refresh function was useful as a smoke test, but range chunks recalculated
-- too much history and timed out on full MLB/NFL coverage. This version refreshes one
-- league + season/year partition at a time, which is the intended production/test unit.
