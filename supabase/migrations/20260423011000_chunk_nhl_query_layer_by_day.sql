...
begin
  ...
  v_start := coalesce(p_start_date, (select min(event_date) from public.ask_goose_nhl_serving_source_v2 where league = 'NHL'));
  v_end := coalesce(p_end_date, (select max(event_date) from public.ask_goose_nhl_serving_source_v2 where league = 'NHL'));
  ...
  while v_cursor <= v_end loop
    ...
    where q.league = 'NHL'
      and q.event_date = v_cursor;

    select count(*) into v_day_total
    from public.ask_goose_nhl_serving_source_v2 s
    where s.event_date = v_cursor;

    v_chunk_start := 1;
    while v_chunk_start <= coalesce(v_day_total, 0) loop
      ...
      select
        ...
        now()
      from ranked r
      where r.rn >= v_chunk_start
        and r.rn < v_chunk_start + v_chunk_size;
