create table if not exists public.ask_goose_loader_source_cache_v1 (
...
create index if not exists ask_goose_loader_source_cache_v1_league_date_idx
  on public.ask_goose_loader_source_cache_v1 (league, event_date);

create or replace function public.refresh_ask_goose_loader_source_cache_v1(
...
begin
  if p_league is null then
    truncate table public.ask_goose_loader_source_cache_v1;
  else
...
begin
  ...
  v_cache_rows := public.refresh_ask_goose_loader_source_cache_v1(p_league, v_final_start, v_final_end);
  ...
  while v_chunk_start <= v_final_end loop
    v_chunk_end := least(v_chunk_start + 2, v_final_end);
