-- Add streak-aware range refresh for Ask Goose game context.

create or replace function public.refresh_game_context_features_v1_range(
  p_league text,
  p_start_date date,
  p_end_date date
)
...
begin
  ...
  joined as (
    ...
    where r.event_date >= p_start_date
      and r.event_date <= p_end_date
  )
  select
