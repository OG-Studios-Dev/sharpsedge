...
if (!sport || !cachePathArg) throw new Error('Usage: node scripts/ingest-the-odds-window.mjs --sport MLB --cache tmp/...json');
...
const normalizeMod = await import(pathToFileURL(path.join(cwd, 'scripts/lib/the-odds-normalize-standalone.mjs')).href);
const mapped = normalizeMod.mapTheOddsHistoricalToGoose2(payload, sport);
...
const candidateRows = dedupe(mapped.candidateRows, 'candidate_id');
