...
const startDate = args.startDate || '2024-04-28';
const endDate = args.endDate || '2026-04-28';
...
function mascot(value) { return norm(value).split(' ').at(-1) || ''; }
function teamMatch(a, b) {
  const aa = norm(a); const bb = norm(b);
  if (!aa || !bb) return false;
  return aa === bb || aa.includes(bb) || bb.includes(aa) || mascot(aa) === mascot(bb);
}
...
async function fetchEspn(day) {
  const res = await fetch(`https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/scoreboard?dates=${espnDate(day)}`);
...
const outPath = path.join(process.cwd(), 'tmp', `ask-goose-nhl-espn-score-backfill-${startDate}-to-${endDate}.json`);
