...
const startDate = args.startDate || '2025-04-01';
const endDate = args.endDate || '2025-11-30';
const dryRun = args.dryRun === '1' || args.dryRun === 'true';
const skipMaterialize = args.skipMaterialize === '1' || args.skipMaterialize === 'true';
...
function teamMatch(a, b) {
  ...
  return aa === bb || aa.includes(bb) || bb.includes(aa) || aa.split(' ').at(-1) === bb.split(' ').at(-1);
}
...
async function fetchEspn(day) {
  const res = await fetch(`https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard?dates=${espnDate(day)}`);
  if (!res.ok) throw new Error(`ESPN ${res.status} ${ymd(day)}`);
  return await res.json();
}
async function fetchDbEvents(day) {
  const q = new URLSearchParams({
    select: 'event_id,sport,league,event_date,commence_time,home_team,away_team,status,event_label,source,source_event_id,odds_api_event_id,metadata',
    league: 'eq.MLB',
    event_date: `eq.${ymd(day)}`,
  });
...
if (!dryRun && !skipMaterialize) {
  ...
  for (const { start, end } of months.values()) {
    const materialized = Number(await rpc('refresh_ask_goose_simple_league_v1', { p_league: 'MLB', p_start_date: start, p_end_date: end }) || 0);
    const graded = Number(await rpc('grade_ask_goose_game_markets_from_event_scores_v1', { p_league: 'MLB', p_start_date: start, p_end_date: end }) || 0);
    const rematerialized = Number(await rpc('refresh_ask_goose_simple_league_v1', { p_league: 'MLB', p_start_date: start, p_end_date: end }) || 0);
...
const outPath = path.join(process.cwd(), 'tmp', `ask-goose-mlb-espn-score-backfill-${startDate}-to-${endDate}.json`);
